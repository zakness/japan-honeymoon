...
import type { SlotItem, SlotItemKind, TransportItemRow } from '@/types/transport'
...
export function useCrossItineraryDnD(cityDays: TripDay[]) {
  ...
  function getItemsForDay(dayDate: string): SlotItem[] {
    ...
    const transport =
      queryClient.getQueryData<TransportItemRow[]>([...TRANSPORT_KEY, dayDate]) ?? []
    const grouped = mergeSlotItems(itinerary, transport)
  ...
  function handleDragStart({ active }: DragStartEvent) {
    ...
    for (const day of cityDays) {
      const found = getItemsForDay(day.date).find((i) => i.data.id === activeId)
      if (found) {
  ...
  function handleDragEnd({ active, over }: DragEndEvent) {
    ...
    if (activeId.startsWith('unscheduled-')) {
      ...
      if (slotTarget) {
        ...
        for (const day of cityDays) {
          const found = getItemsForDay(day.date).find((i) => i.data.id === overId)
          if (found) {
            targetDayDate = day.date
            targetSlot = found.data.time_slot as TimeSlot
            break
          }
        }
      }
      ...
      const slotItems = getItemsForDay(targetDayDate).filter((i) => i.data.time_slot === targetSlot)
      createItem.mutate({
    ...
    if (slotTarget) {
      ...
      let found = false
      for (const day of cityDays) {
        const item = getItemsForDay(day.date).find((i) => i.data.id === overId)
        if (item) {
          targetDayDate = day.date
          targetSlot = item.data.time_slot as TimeSlot
          found = true
          break
        }
      }
    ...
    if (sourceDayDate === targetDayDate!) {
      const grouped = mergeSlotItems(
        ...
        sourceItems.filter((i) => i.kind === 'transport').map((i) => i.data as TransportItemRow)
      )
      const sourceSlotItems = grouped[sourceSlot]
      const targetSlotItems = grouped[targetSlot!]

      if (sourceSlot === targetSlot!) {
        const oldIndex = sourceSlotItems.findIndex((i) => i.data.id === activeId)
        const newIndex = slotTarget
          ? sourceSlotItems.length - 1
          : sourceSlotItems.findIndex((i) => i.data.id === overId)
        ...
        const insertAt = slotTarget
          ...
          : targetSlotItems.findIndex((i) => i.data.id === overId)
        const movedItem = {
          ...sourceItems.find((i) => i.data.id === activeId)!,
          data: {
            ...sourceItems.find((i) => i.data.id === activeId)!.data,
            time_slot: targetSlot!,
          },
        } as SlotItem
    ...
    const sourceSlotItems = sourceItems.filter((i) => i.data.time_slot === sourceSlot)
    const targetSlotItems = targetItems.filter((i) => i.data.time_slot === targetSlot!)

    const fromSlotUpdates: DayReorderItem[] = sourceSlotItems
      .filter((i) => i.data.id !== activeId)
      .map((item, idx) => ({
        id: item.data.id,
        kind: item.kind,
        sort_order: idx,
    ...
    newTargetSlot.splice(insertAt, 0, sourceItems.find((i) => i.data.id === activeId)!)
