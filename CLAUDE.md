...
**Hotels** — `accommodations` rows carry the same rich-place fields as places (`google_place_id`, `photos`, `rating`, `tags`, `notes`, `phone`) on top of stay-specific columns (dates, times, confirmations, `booked_by`, `booking_url`). Time columns come in two pairs: `check_in_time` / `check_out_time` are the **planned** arrival / departure (when we'll actually be at the hotel); `check_in_policy_time` / `check_out_policy_time` are the **hotel's policy** (earliest check-in allowed / latest check-out — copied from the booking confirmation). Both pairs are optional; `formatHotelTimePill` (`src/types/itinerary.ts`) renders the 4-state pill (`6:00 PM`, `6:00 PM (from 3:00 PM)`, `from 3:00 PM`, `time TBD`) consumed by the day-column card, logistics row, and detail panel. `AppShell` lifts `selectedHotel` and `editingHotel` state alongside the place equivalents, with mutual exclusion enforced in `handleSelectPlace` / `handleSelectHotel`. Selecting a hotel renders the shared `DetailPanel` (sibling to `CityMap`) with `kind: 'hotel'` so place and hotel detail surfaces are visually identical. `HotelEditDialog` is mounted once at AppShell level and driven by `editingHotel`; the Edit affordance lives in the detail panel and on every Logistics `HotelEntry` (both check-in and check-out). Hotels remain edit-only — creation and deletion still happen via SQL/seed migrations.

**Hotel events (check-in / check-out)** — each stay materializes two first-class `itinerary_items` rows discriminated by `accommodation_id` + `hotel_event_role` (`'checkin' | 'checkout'`), alongside the existing `place` / `text_note` kinds. Lifecycle is enforced at the data layer: an AFTER INSERT trigger on `accommodations` creates the pair (deriving `time_slot` from `derive_hotel_slot(planned, policy, default)` where defaults are `evening` for check-in and `morning` for check-out); an AFTER UPDATE trigger syncs the items' `day_date` when stay dates change (but **never** touches `time_slot` or `sort_order` — user layout choices win after first placement); `ON DELETE CASCADE` removes them when the stay is deleted. Defined in `supabase/migrations/018_hotel_event_items.sql`. The DB content check requires exactly one of (`place_id`, `text_note`, `accommodation_id`) per row; `itemKind()` (`src/types/itinerary.ts`) is the canonical discriminator. `ItineraryItem` dispatches to `HotelEventCard` (`src/components/day/HotelEventCard.tsx`) when the kind is a hotel event — no banner, leading `LogIn` / `LogOut` icon, hotel-tinted, always rendered `decided`; resolves the linked accommodation via the cached `useAccommodations()` list. Action tray is Edit-only (no Delete — check-in / check-out can't exist without the stay; route deletions through the hotel). Cross-day drag is rejected by `useCrossItineraryDnD` for hotel-event items because changing a card's day would imply moving the stay's date, which is a deliberate `HotelForm` edit. Intra-day reorder is unrestricted.
