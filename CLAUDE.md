...
**Itinerary view — desktop** — `src/components/itinerary/ItineraryView.tsx`. Horizontally scrollable day columns (one per day in the selected city) plus a sticky Unscheduled column on the left, `CityMap` on the right. City strip at top switches between the 5 cities. DnD via `useCrossItineraryDnD` (`src/hooks/`) supports same-day, cross-slot, cross-day, and unscheduled→day moves.

**Itinerary view — mobile** — fundamentally different layout, gated by `useIsDesktop()` (640px breakpoint). Map fills the viewport; a draggable `BottomSheet` (`ItineraryView.tsx` internal) overlays the bottom with two snap points (collapsed ~32vh, expanded ~85vh) — drag the grabber pill to switch. Inside the sheet: `CityStrip` → `DayStrip` → a single `DayColumn` (or `UnscheduledColumn` when the `Places` tab is active). `DayStrip` is a horizontal scroller of the city's days plus a right-pinned `Places` tab; the selected day uses the city's `tint`, edges fade via `mask-image` when overflowing. The single visible day is wrapped in `DayColumnSwiper`, which advances ±1 trip day on horizontal swipe (crosses city boundaries via `onNavigate` to the new city's URL); peek-card stubs at the edges hint at adjacent days. Only one day mounts at a time — cross-day DnD is naturally impossible on mobile; within-day reorders still work. `DayColumn` and `UnscheduledColumn` accept a `fillWidth` prop for the mobile single-column layout. `HotelAnchor` accepts a `bleed` prop so morning/evening hotel banners sit flush to the day card's rounded edges. Selecting a place/hotel/journey replaces the sheet with a fixed ~62vh detail sheet (golden ratio: map keeps ~38vh, no grabber since it isn't draggable); `CityMap` suppresses its floating overlay cards on mobile (`showFloatingCards={false}`) to avoid duplicating the sheet content. `smoothPanTo` biases the camera south by `bottomPadPx / 2` on mobile (vs. the desktop top-right card bias) so the selected marker lands above the sheet. Initial city framing uses `fitBounds` over all the city's pins (places + hotels) with `bottom: 60 + bottomPadPx` padding; the map is NOT re-fit when the sheet snap toggles — the user's camera is theirs to control after the initial frame.
...
**Unified place selection** — `AppShell` owns `selectedPlace` / `selectionOrigin` state. Three click surfaces converge: backlog card (`'backlog'`), map marker (`'marker'`), and day-column card (`'day-column'` — the whole card body is clickable). The overloaded `SelectPlaceHandler` type forces callers to provide an origin when setting a real place. Clicking a backlog card while the map is hidden auto-reveals it. On desktop, `CityMap` renders a floating `PlaceDetailCard` over the map when a place is selected; on mobile, a ~62vh detail sheet replaces the itinerary sheet (and the floating card is suppressed via `showFloatingCards={false}` to avoid duplication). `UnscheduledColumn` highlights and auto-scrolls to the selected card (skipping scroll when origin is `'backlog'`). `PlaceEditDialog` is mounted once at AppShell level, driven by `editingPlace` state. `useScheduledDatesByPlace` is the bulk query powering marker badges — prefer it over the per-place `usePlaceSchedule` in list/map contexts.
